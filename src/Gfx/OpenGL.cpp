...
void Renderer::render_internal()
{
    ...
    for (auto const* buffer : m_render_buffers) {
        ...
        while ((renderBufferChunk != nullptr) && (pos < renderBufferChunk->used)) {
            ...
            case RenderItemType::SetTexture: {
                ...
                if (header->texture == nullptr) {
                    ...
                    Texture& texture = reinterpret_cast<DeprecatedAsset&>(*header->texture->loaded_asset).texture;
...
void Renderer::after_assets_loaded()
{
    ...
    for (auto it = asset_manager().assetsByType[AssetType::Texture].iterate();
        ...
        auto& texture = reinterpret_cast<DeprecatedAsset&>(*asset->loaded_asset).texture;
    ...
    for (auto it = asset_manager().assetsByType[AssetType::Shader].iterate();
        ...
        shader->asset = asset;
        reinterpret_cast<DeprecatedAsset&>(*asset->loaded_asset).shader.rendererShaderID = shaderIndex;
...
void Renderer::before_assets_unloaded()
{
    ...
    for (auto it = asset_manager().assetsByType[AssetType::Texture].iterate();
        ...
        it.next()) {
        AssetMetadata* asset = *it.get();
        Texture& texture = reinterpret_cast<DeprecatedAsset&>(*asset->loaded_asset).texture;
...
void loadShaderProgram(AssetMetadata* asset, ShaderProgram* glShader)
{
    ...
    if (glShader->shaderProgramID) {
        auto& shader = reinterpret_cast<DeprecatedAsset&>(*asset->loaded_asset).shader;
