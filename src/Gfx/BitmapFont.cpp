...
{
    return reinterpret_cast<DeprecatedAsset&>(*getAsset(AssetType::BitmapFont, name.deprecated_to_string()).loaded_asset).bitmapFont;
}
...
bool BitmapFont::load_from_bmf_data(Blob data, AssetMetadata& metadata, DeprecatedAsset& asset)
{
    ...
    if (!(common && chars && charCount && pages)) {
        ...
        auto& texture = reinterpret_cast<DeprecatedAsset&>(*font->m_texture->loaded_asset);
