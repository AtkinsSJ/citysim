...
bool loadSaveFile(FileHandle *file, GameState *gameState)
{
	...
	while (reader.isValidFile)
	{
		...
		if (!loadTerrainLayer(&city->terrainLayer, city, &reader)) break;
		if (!loadBuildings(city, &reader)) break;
		if (!loadZoneLayer(&city->zoneLayer, city, &reader)) break;
		if (!loadCrimeLayer(&city->crimeLayer, city, &reader)) break;
		if (!loadEducationLayer(&city->educationLayer, city, &reader)) break;
		if (!loadFireLayer(&city->fireLayer, city, &reader)) break;
		if (!loadHealthLayer(&city->healthLayer, city, &reader)) break;
		if (!loadLandValueLayer(&city->landValueLayer, city, &reader)) break;
		if (!loadPollutionLayer(&city->pollutionLayer, city, &reader)) break;
		if (!loadTransportLayer(&city->transportLayer, city, &reader)) break;
		if (!loadBudgetLayer(&city->budgetLayer, city, &reader)) break;
