...
void drawDataViewUI(GameState *gameState)
{
	...
	if (UI::isMenuVisible(Menu_DataViews))
	{
		...
		UIPanel menu = UIPanel(irectAligned(dataViewButtonBounds.x - popupMenuPanelStyle->margin, dataViewButtonBounds.y, popupMenuWidth, popupMenuMaxHeight, ALIGN_BOTTOM | ALIGN_LEFT), popupMenuPanelStyle, Panel_BlocksMouse | Panel_LayoutBottomToTop);
	 ...
	 && gameState->dataLayerToDraw != DataView_None)
	{
		...
		UIPanel ui = UIPanel(irectAligned(uiPos.x, uiPos.y, 240, 1000, ALIGN_BOTTOM | ALIGN_LEFT), null, Panel_BlocksMouse | Panel_LayoutBottomToTop);
		{
