...
void updateAndRenderGame(AppState *appState, InputState *inputState, Renderer *renderer, AssetManager *assets)
{
			...
			switch (buildingDef->buildMethod)
			{
				case BuildMethod_Paint:
				{
					if (!mouseIsOverUI)
					{
						...
						drawSprite(&renderer->worldBuffer, getSprite(assets, getSpriteID(assets, buildingDef->spriteType, 0)),
		...
		y++)
	{
			...
			x++)
		{
			...
			if (t.type != 0)
			{
				...
				drawSprite(&renderer->worldBuffer, getSprite(assets, spriteID), rectXYWH((f32)x, (f32)y, 1.0f, 1.0f), -1000.0f, makeWhite(), Shader_PixelArt);
			}
		}
	}
	...
	for (auto it = iterate(&city->buildings, 1, false); !it.isDone; next(&it))
	{
		...
		if (rectsOverlap(building->footprint, visibleTileBounds))
		{
			...
			drawSprite(&renderer->worldBuffer, getSprite(assets, getSpriteID(assets, def->spriteType, building->spriteOffset)),
