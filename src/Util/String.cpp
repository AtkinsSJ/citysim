...
Optional<s64> String::to_int() const
{
    ...
    for (s32 position = start_position; position < length; position++) {
        value *= 10;
...
Optional<double> String::to_double() const
{
    // TODO: Implement this properly!
    // (c runtime functions atof / strtod don't tell you if they failed, they just return 0 which is a valid value!
    String null_terminated_input = pushString(&temp_arena(), length + 1);
    copyString(*this, &null_terminated_input);
    null_terminated_input.length++;
    null_terminated_input.chars[length] = '\0';

    // FIXME: Use strtod() to avoid this mess.
    double double_value = atof(null_terminated_input.chars);
    if (double_value == 0.0) {
...
String myprintf(String format, std::initializer_list<String> args, bool zeroTerminate)
{
    ...
    for (s32 i = 0; i < format.length; i++) {
        if (format.chars[i] == '{') {
            ...
            if (indexLength > 0) {
                // Positional
                String indexString { format.chars + startOfNumber, (size_t)indexLength };

                if (auto parsedIndex = indexString.to_int(); parsedIndex.has_value()) {
