...
void Drawable::preparePlaceholder(RenderBuffer* buffer)
{
    ...
    case DrawableType::Ninepatch: {
        placeholder = appendDrawNinepatchPlaceholder(buffer, reinterpret_cast<DeprecatedAsset&>(*style->ninepatch.get().loaded_asset).ninepatch.texture, renderer.shaderIds.textured);
    } break;
...
void Drawable::fillPlaceholder(Rect2I bounds)
{
    ...
    case DrawableType::Ninepatch: {
        fillDrawNinepatchPlaceholder(&placeholder.get<DrawNinepatchPlaceholder>(), bounds, &reinterpret_cast<DeprecatedAsset&>(*style->ninepatch.get().loaded_asset).ninepatch, style->color);
    } break;
...
void Drawable::draw(RenderBuffer* buffer, Rect2I bounds)
{
    ...
    case DrawableType::Ninepatch: {
        drawNinepatch(buffer, bounds, renderer.shaderIds.textured, &reinterpret_cast<DeprecatedAsset&>(*style->ninepatch.get().loaded_asset).ninepatch, style->color);
    } break;
