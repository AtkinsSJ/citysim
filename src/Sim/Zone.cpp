...
bool isZoneAcceptable(City* city, ZoneType zoneType, s32 x, s32 y)
{
    ...
    if (city->building_exists_at(x, y))
        return false;
    if (city->transportLayer.distance_to_transport(x, y, TransportType::Road) > def.maximumDistanceToRoad)
        return false;
                    ...
                    while (true) {
                        ...
                        if (tryX) {
                            ...
                            if (canExpand) {
                                ...
                                if (!positive)
                                    zoneFootprint.set_x(zoneFootprint.x() - 1);
                            }
                        } else // expand in y
                        {
                            ...
                            if (canExpand) {
                                ...
                                if (!positive)
                                    zoneFootprint.set_y(zoneFootprint.y() - 1);
                            }
                        }
