...
static void loadTexts(HashTable<String>* texts, AssetMetadata* metadata, Blob file_data)
{
...
ErrorOr<NonnullOwnPtr<Asset>> DeprecatedAssetLoader::load_asset(AssetMetadata& metadata, Blob file_data)
{
    ...
    case AssetType::Cursor: {
        auto image_data = readTempFile(asset->cursor.imageFilePath);
    ...
    case AssetType::CursorDefs: {
        loadCursorDefs(file_data, metadata, *asset);
        return { move(asset) };
    }
    ...
    case AssetType::Ninepatch: {
        ...
        t->ensure_is_loaded();
        auto& texture = reinterpret_cast<DeprecatedAsset&>(*t->loaded_asset).texture;
    ...
    case AssetType::Palette: {
        ...
        case Palette::Type::Gradient: {
            asset->data = assetsAllocate(&asset_manager(), palette->size * sizeof(Colour));
    ...
    case AssetType::PaletteDefs: {
        loadPaletteDefs(file_data, metadata, *asset);
        return { move(asset) };
    }
    ...
    case AssetType::SpriteDefs: {
        loadSpriteDefs(file_data, metadata, *asset);
        return { move(asset) };
    }
    ...
    case AssetType::Texts: {
        ...
        loadTexts(textsTable, &metadata, file_data);
        return { move(asset) };
    }
